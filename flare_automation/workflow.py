...
@dataclass
class RunContext:
    """Holds the capture/analysis directories for a workflow execution."""
...
class FlareCaptureWorkflow:
    ...
    def _prepare_output(self) -> RunContext:
        ...
        self.run_context = RunContext(root=run_dir, capture_dir=capture_dir, analysis_dir=analysis_dir)
    ...
    def run(self) -> Iterator[CaptureResult]:  # pragma: no cover - orchestrates hardware
        ...
        with self.serial_controller.open(serial_port) as connection:
            ...
            for illumination, sequence in self.config.iter_sequences():
                ...
                illumination_dir = context.capture_dir / illumination.name
