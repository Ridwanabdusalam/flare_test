...
2. **Python packages**: Install the runtime dependencies with pip:
   ...
   * `numpy` powers the RAW16 math used by the analysis workflow.
   * Optional: `matplotlib` is used to render ROI preview images; if it is missing the
     workflow still runs but emits a small JSON note instead of a PNG overlay.
...
4. **Run the workflow**:
   ...
   ```
5. Monitor the console output; a summary line is printed for each illumination/exposure combination captured. When the run finishes, the output directory contains a `capture/` tree with raw footage and metadata. If the configuration includes an `analysis` block, the tool immediately computes ROI statistics and flare metrics under an `analysis/` sibling directory.
...
Provide this block to enable automated post-processing after captures complete:

```yaml
analysis:
  enabled: true
  sequence_label: visible                # Exposure sequence to analyze
  nd_filter_ratio: 0.1071                # Ratio used to normalize the direct beam ROI
  saturation_dn: 600                     # Threshold to exclude saturated points
  scenes:
    - illumination: camera_led
      illumination_lux: 587
  rois:
    - name: reflectance_high
      row: 1200
      col: 1800
      half_width: 8
      role: reflectance
...
* `sequence_label` – The exposure sequence to pull frames from when computing ROI statistics.
* `scenes` – Each scene maps an illumination name to a calibrated illumination value (lux). The order determines how slopes are fitted versus illumination.
* `rois` – Rectangular regions (center coordinates with half-width) used for statistics. Assign one ROI the `direct_beam` role (and optionally one `black_hole`); all other regions intended for the reflectance fit should use `role: reflectance` and include a `reflectance` value.
* `nd_filter_ratio` – Normalization applied to the direct beam ROI before computing its slope.
* `saturation_dn` – Samples above this DN are omitted from the first-stage linear regression.
* `preview` – Optional hint that tells the workflow which frame to render with ROI overlays. If omitted, a JSON note is recorded instead of an image.
...
5. Iterates through each exposure value:
   ...
   * Pulls the generated `.raw` files into `<run>/capture/<illumination>/<label>_<exposure>us/raw10/`.
...
7. If the configuration contains an `analysis` block, the workflow immediately loads the converted RAW16 frames, measures each ROI, fits photo-response curves, and saves the results under `<run>/analysis/`.
...
<output_root>/
  sceneName_YYYYMMDD_HHMMSS/
    ...
    analysis/
      roi/
        rois.json
        roi_preview.png          # Only when matplotlib is installed
      metrics/
        roi_signals.json         # Mean DN per ROI/exposure/scene
        photo_response.json      # Linear fit coefficients
      summary.json
```

This layout separates the “pre-analysis” artifacts (`capture/`) from the derived products (`analysis/`) so you can archive or reprocess either side independently. All metadata is stored as JSON so downstream processing can reconstruct the capture context without inspecting folder names.
